 ...
 * the page reveals content as a function of scroll position, so an unsettled
 * capture would photograph everything below the first viewport in its ghosted
 * state — and the critic would score the reveal system as washed-out design.
 *
...
export async function settleReveals(page) {
  ...
  // Reveals latch, so one downward pass is enough; this covers late layout
  // shifts from images that only decoded during the pass.
